...
public class UserFilter extends RowFilter {
	...
	public void applyFilters(ExportContext context) {
		if (!getLimitToUsers().isEmpty()) {
			...
			ForeignKeyTransform foreignKeyTransform = new ForeignKeyTransform();
			foreignKeyTransform.setReferencedTable("users");
			foreignKeyTransform.setReferencedColumn("user_id");
			registerTransform(foreignKeyTransform, context);
		}
	}
